...
3. **lintエラーの事前チェックと解決**
   - **リファクタリング提案を行う前に、必ず対象ファイルと関連ファイルのlintエラーをチェックする**
   - lintエラーが存在する場合、先にlintエラーを解決してからリファクタリング提案を行う
   - 提案するサンプルコードもlintエラーがないことを確認する
   - 新規作成するファイルもlintエラーがないことを確認する
...
### Step 0: lintエラーの事前チェック

**重要**: リファクタリング提案を行う前に、必ず以下の手順を実行してください。

1. **対象ファイルのlintエラーチェック**
   - `read_lints`ツールを使用して対象ファイルのlintエラーを確認
   - 関連ファイル（インポート/エクスポートされているファイル）もチェック

2. **lintエラーの解決**
   - lintエラーが存在する場合、先にlintエラーを解決する
   - lintエラーが解決できない場合は、リファクタリング提案に含める

3. **サンプルコードのlintチェック**
   - 提案するサンプルコードもlintエラーがないことを確認
...
1. **提案前のlintエラーチェック（必須）**
   - **リファクタリング提案を行う前に、必ず対象ファイルと関連ファイルのlintエラーをチェックする**
   - lintエラーが存在する場合、先にlintエラーを解決してからリファクタリング提案を行う
   - `read_lints`ツールを使用してlintエラーを確認する
...
1. **対象ファイルのlintエラーチェック**
   ```typescript
   // read_lintsツールを使用してlintエラーを確認
   read_lints({ paths: ['対象ファイルのパス'] })
   ```

2. **関連ファイルのlintエラーチェック**
   - インポート/エクスポートされているファイルもチェック
   - 新規作成する予定のファイルのコードもlintエラーがないことを確認
...
**重要**: ユーザーに提案する前に、以下を確認してください。

1. **対象ファイルのlintエラーが解決されているか確認**
2. **提案するサンプルコードにlintエラーがないか確認**
3. **新規作成するファイルのコードにlintエラーがないか確認**
...
- [ ] **対象ファイルと関連ファイルのlintエラーをチェックした**
- [ ] **lintエラーが存在する場合、先に解決した**
...
1. **lintエラーの事前チェック（必須）**
   - 対象ファイルと関連ファイルのlintエラーをチェック
   - lintエラーが存在する場合、先に解決してからリファクタリング提案を行う
   - `read_lints`ツールを使用してlintエラーを確認する
