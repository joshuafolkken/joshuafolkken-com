...
2. For each issue `#<N>` in the supplied order:
   ...
   b. Execute the full `fullrun #<N>` flow: normalize title → add `in-progress` label → post plan if body is blank → implement → run the verification gate (refactor → `pnpm josh gate` (lint, type check, spell check and unit tests, run concurrently) → `/code-review` at the level `pnpm josh review:level` prints on `git diff main`, iterating until no high/medium findings remain — **at most two reviews in total** (`node_modules/@joshuafolkken/kit/prompts/review.md` → "Review round cap") → `pnpm josh eval:scope`, and `pnpm josh eval` when it answers `required` (`eval-gate.md`)) → `pnpm josh bump minor` → `pnpm josh git -y "<title> #<N>"` → `pnpm josh followup "<title> #<N>" --merge --notify-message "Implemented <title>\nCause: ...\nFix: ...\nResult: ...\n\nDetails:\n- ..."` (sends per-issue completion notification and merges, exactly as `fullrun` does) → `pnpm josh ms` (return to the default branch).
...
- **An issue carrying `needs-human-review` ends the queue before its commit.** Implement it and take
  ...
  (canonical: `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/human-review-label.md`).
