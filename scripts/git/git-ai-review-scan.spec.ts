...
function make_comment(input: {
	...
	return { author_login: input.author_login, body: input.body, url: input.url ?? COMMENT_URL }
}
...
describe('classify_ai_review_comments — CodeRabbit', () => {
	it('flags "Actionable comments posted: N" (N > 0) as blocker', () => {
		const result = classify_ai_review_comments([
			make_comment({ author_login: CODERABBIT, body: CODERABBIT_ACTIONABLE_BODY }),
		])
	...
	it('does not flag rate-limit notice', () => {
		const result = classify_ai_review_comments([
			make_comment({ author_login: CODERABBIT_BOT, body: CODERABBIT_RATE_LIMIT_BODY }),
		])
