...
const HEADER_LEFT_SECTION_CLASSES = 'flex min-w-0 flex-1 items-center gap-4 desktop:gap-6'
...
const SEARCH_BUTTON_CLASSES = `${SOCIAL_LINK_DESKTOP_CLASSES} cursor-pointer`

const MENU_TOGGLE_BUTTON_CLASSES = `cyber-glow-hover ${ICON_BUTTON_BASE} desktop:hidden ${INTERACTIVE_SCALE_HOVER}`
...
const SOCIAL_LINK_CONTAINER_DESKTOP = 'hidden items-center gap-2 desktop:flex'
const SOCIAL_LINK_CONTAINER_MOBILE = 'mt-4 flex gap-2 p-4 pt-0 desktop:hidden'

const HEADER_PAGE_LINK_BASE =
	'group flex min-w-0 items-center gap-2 text-base font-medium text-white/80 desktop:hidden [&_svg]:text-inherit'
...
const HEADER_LOGO_TEXT_HIDDEN = 'max-w-0 opacity-0 desktop:max-w-xs desktop:opacity-100'
