...
function stub_fetch(response: Response): void {
...
describe('chat_api.ask', () => {
	it('returns grounded:false when the endpoint responds with json', async () => {
		...
		const outcome = await chat_api.ask(QUESTION, on_token)
	...
	it('throws when the response is not ok', async () => {
		...
		await expect(chat_api.ask(QUESTION, vi.fn())).rejects.toThrow()
	})
})
...
describe('chat_api.ask streaming', () => {
	it('streams tokens and returns grounded:true for an event stream', async () => {
		...
		const outcome = await chat_api.ask(QUESTION, (text) => {
			tokens.push(text)
		})
	...
	it('emits a final data line that is not terminated by a newline', async () => {
		...
		await chat_api.ask(QUESTION, (text) => {
			tokens.push(text)
		})
