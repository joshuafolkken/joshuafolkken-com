...
The canonical extended reference is `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/` (indexed by `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow.md`); this skill is the
...
- **The verification gate**, in this order: refactor per `node_modules/@joshuafolkken/kit/prompts/refactoring.md` → `pnpm josh gate` (lint, type check, spell check and unit tests, run concurrently) → `/code-review` at the level `pnpm josh review:level` prints
  on `git diff main`, iterating until no high/medium findings remain — **at most two reviews in total**
  (`node_modules/@joshuafolkken/kit/prompts/review.md` → "Review round cap") → `pnpm josh eval:scope`, and `pnpm josh eval` when it
  ...
  **E2E closes after that, and never by asking the user**: where the command ends in a pull request
  ...
  the stop. `CLAUDE.md` → "Completion gate" carries the rule, `node_modules/@joshuafolkken/kit/prompts/testing-guide.md` → "Closing
...
Canonical reference: `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/human-review-label.md`.
...
Canonical reference: `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/into-epic.md`.
...
Canonical reference: `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/delegation.md`.
...
Canonical reference: `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/target-repo.md`.
...
2. **The pointer** — the topic file under `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/` or the skill file that holds
...
this skill is, and one entry below routes to `node_modules/@joshuafolkken/kit/prompts/review.md`, which is not a skill at all.
...
- **The follow-up filing step after the review round cap** — filing the Issue and bundling it into an
  ...
  Its full form is in `node_modules/@joshuafolkken/kit/prompts/review.md` → "Review round cap" (joshuafolkken/kit#946).
