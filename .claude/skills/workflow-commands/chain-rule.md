...
| One or more High / Medium findings      | High and/or Medium | Fix in place and re-run `/code-review` at the level `pnpm josh review:level` prints on `git diff main` — **at most two reviews in total** (`node_modules/@joshuafolkken/kit/prompts/review.md` → "Review round cap"). Nothing is committed yet, so a round costs no commit, push, or CI run. Do NOT report narratively and wait. |
...
See `node_modules/@joshuafolkken/kit/prompts/collaboration-workflow/plan-comment.md` → "Chain rule: `/code-review` → `followup --merge` decision table" for the canonical extended reference.
