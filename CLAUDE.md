...
- **If the user wants to skip the merge step**, use `kickoff` (plan-only) or explicitly say "do not merge" / "do not auto-merge" in the same turn. Outside of a `fullrun` invocation, never run `gh pr merge` on your own.
