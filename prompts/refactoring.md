...
4. **リファクタリング提案を実行**
   - 特定したファイル（および関連・類似ファイル）のリファクタリング案を3つまで提案
   - 実装可能なサンプルコードを提供
   - コメント提案は不要
...
3. **関連・類似ファイルの確認**
   - 特定したファイルを中心に分析
   - 関連するファイル（インポート関係、同じディレクトリ、類似機能、依存関係）を特定して確認
   - 関連しているものや似たものがあれば、そちらもリファクタリング対象に含める
...
1. **選択・開いているファイルがある場合** → そのファイルを中心に分析（除外条件に該当する場合は除外）
2. **選択・開いているファイルがない場合** → 変更ファイル（`git status` の M/A/D 等）を確認し、それを中心に分析
3. **変更ファイルがない場合** → 現在のブランチの変更内容（main 等との差分）を確認し、それを中心に分析
4. **ブランチ変更もない場合**（main ブランチ等）→ プロジェクト全体を分析し、最も改善が必要なファイルを特定

**関連・類似ファイルの拡張**: 上記で特定したファイルを中心に、関連しているものや似たものがあればそちらもリファクタリング対象に含める。
...
**関連・類似ファイルの拡張**: 特定したファイルを中心に、関連しているものや似たものがあればそちらもリファクタリング対象に含める。
...
```bash
# 関連・類似ファイルを探す（これらもリファクタリング対象に含める）
- インポートされているファイル
- 同じディレクトリのファイル
- 類似機能のファイル
- 依存関係のあるファイル
...
**重要**: 「リファクタリングを提案して」と指示された場合、以下の手順でリファクタリング提案を実行してください：

1. **リファクタリング対象の特定**
   - 選択・開いているファイルがある場合 → そのファイルを中心に分析（除外条件に該当する場合は除外）
   - 選択・開いているファイルがない場合 → 変更ファイル（`git status`）を確認し、それを中心に分析
   - 変更ファイルがない場合 → 現在のブランチの変更内容（main 等との差分）を確認し、それを中心に分析
   - ブランチ変更もない場合（main ブランチ等）→ プロジェクト全体を分析し、最も改善が必要なファイルを特定
