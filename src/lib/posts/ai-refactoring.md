...
以前の記事では説明が抜けていたんですが、僕が AI を使うときは基本的に **Cursor** を使っています。ごめんなさいー！YouTubeのライブ配信中に質問されて「Cursor 使ってるって書いてなかったわ！」って気づきました。
...
今回、AI の提案で TechStack データ定義をファイルに分割したけど、SonarCube さんに、「重複コードありすぎー！こんな分け方あかんやろ！」って怒られました。

TechStack のデータを 12 個のファイルに分割したのは良かったんですが、各ファイルが同じ構造（`import`、`export`、オブジェクト形式）を持っていたため、SonarCube が重複コードとして検出してしまったんです。

最初は共通のヘルパー関数を作って対応しようとしましたが、それでも重複の警告パーセンテージはほとんど変わりませんでした。根本的にデータの定義方法を変えないといけないと気づきました。

そこで、すべてのデータを 1 つのファイルに集約し、タプル形式 `[name, url]` でコンパクトに定義する方法に変更しました。これにより、SonarCube さんからの指摘がなくなりました。

SonarCube さんのチェックはめっちゃ厳しいんですよ。だかそれがいい！厳しい先生、最高です。
