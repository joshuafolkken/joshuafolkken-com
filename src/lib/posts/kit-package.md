...
最初は `@joshuafolkken/config` という名前でした。作ってみたら即バグ祭りで、最初の10個の Issue がほぼ全部「Fix 〜」でした。今思い返しても笑えない（笑）。パッケージ名を `kit` に改名してからも改善を続けて、現在 **v0.55.0**。マイナーバージョンを**55回**上げてきました。
...
ただ、ちゃんと動くことは確認できています。このブログ（joshuafolkken.com のプロジェクト）自体が、もともと一体化していた kit 部分を引き剥がして外部パッケージ化したものなんです。つまり、このサイト自体が**最初のユーザー**です。数々の調整を経て、いまは `@joshuafolkken/kit` を使いながらちゃんと動いていることを確認できました。**v0.55.0 まで来て、55回更新してやっと使えるようになった**——そういうことです。
